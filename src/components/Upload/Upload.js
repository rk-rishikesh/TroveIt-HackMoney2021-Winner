...
class Upload extends Component {
  ...
  async loadBlockchainData() {
    ...
    if (networkData) {
      ...
      const originalPostCount = await trove.methods
        .originalPostCount()
        .call();
      ...
      for (var i = 1; i <= originalPostCount; i++) {
        const originalPost = await trove.methods.originalPosts(i).call();
        this.setState({
          originalPosts: [...this.state.originalPosts, originalPost],
        });
      }

      const feedPostCount = await trove.methods.feedPostCount().call();
  ...
  render() {
    ...
    return (
      <div
        style={{ width: "100%", height: "80%", backgroundImage: `url(${bg})` }}
      >
        {this.state.loading ? (
